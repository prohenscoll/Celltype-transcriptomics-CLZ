############################################
# 01_Zeisel2015_scRef_forCIBERSORTx.R
# Single-cell RNA-seq reference construction (Zeisel et al., 2015)
#
# - Load Zeisel 2015 scRNA-seq dataset
...
out_file <- file.path(out_dir, "Zeisel2015_scRef_major5_forCIBERSORTx.tsv")
...
############################################
## 1. Load Zeisel 2015 scRNA-seq dataset
############################################
zeisel <- ZeiselBrainData()
message("Cell counts by original level1class:")
print(table(zeisel$level1class))

############################################
## 2. Convert to Seurat object
############################################
seurat_obj <- as.Seurat(zeisel, counts = "counts", data = NULL)
DefaultAssay(seurat_obj) <- "gene"

############################################
## 3. Map level classes to major cell types (major5)
############################################
seurat_obj$major5 <- dplyr::case_when(
  seurat_obj$level1class %in% c("interneurons", "pyramidal CA1", "pyramidal SS") ~ "Neuron",
  seurat_obj$level1class %in% c("oligodendrocytes", "oligodendrocyte precursors") ~ "Oligodendrocyte",
  seurat_obj$level1class %in% c("astrocytes_ependymal", "astrocytes") ~ "Astrocyte",
  seurat_obj$level1class %in% c("microglia") ~ "Microglia",
  seurat_obj$level1class %in% c("endothelial-mural", "endothelial", "mural") ~ "Endothelial",
  TRUE ~ NA_character_
)
...
############################################
seurat_obj <- subset(seurat_obj, cells = colnames(seurat_obj)[!is.na(seurat_obj$major5)])
...
############################################
if ("counts" %in% Layers(seurat_obj, assay = "gene")) {
  mat <- GetAssayData(seurat_obj, assay = "gene", layer = "counts")  
} else {
  mat <- GetAssayData(seurat_obj, assay = "gene", slot = "counts")   
}
mat <- mat[rowSums(mat) > 0, ]
mat <- mat[!grepl("^ERCC-", rownames(mat)), ]
message("Matrix dimensions after filtering (genes x cells): ",
